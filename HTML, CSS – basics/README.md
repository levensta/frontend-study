## Основы HTML и CSS

### 1. Введение

* [Анатомия HTML](https://developer.mozilla.org/ru/docs/Learn/HTML/Introduction_to_HTML/Getting_started)
* [Шапка head](https://developer.mozilla.org/ru/docs/Learn/HTML/Introduction_to_HTML/The_head_metadata_in_HTML)
* [Микроразметка schema.org](https://yandex.ru/support/webmaster/schema-org/intro-schema-org.html?lang=ru)
* [Таблицы](https://html5book.ru/html-table/)

### 2. Виды элементов

* [Основные и семантические элементы](https://developer.mozilla.org/ru/docs/Web/HTML/Element)
* [Ссылки](https://html5book.ru/hyperlinks-in-html/)
* [Изображения](https://html5book.ru/images-in-html)
* [Формы (кнопки, чекбоксы и т.д.)](https://html5book.ru/html5-forms/)
* [Тег form](http://htmlbook.ru/html/form)

### 3. Введение в CSS

* [Селекторы](https://developer.mozilla.org/ru/docs/Web/CSS/CSS_Selectors)
* [Отношения селекторов](https://webref.ru/layout/advanced-html-css/complex-selectors)
* [Псевдоэлементы](http://htmlbook.ru/samcss/psevdoelementy)
* [Псевдоклассы](http://htmlbook.ru/css/cat/pseudoclass)
* [Специфичность](https://developer.mozilla.org/ru/docs/Web/CSS/Specificity)
